...
model1 = lm(price ~ distance + surge_multiplier + name + distance:surge_multiplier, refined_data)
...
-   **Result**:
    ...
    -   **BP Statistic = 64.976**, **p-value = 3.45e-08**.

    -   The null hypothesis of **homoscedasticity** (constant variance) is still rejected.

    -   Although heteroscedasticity has likely decreased compared to earlier results (BP = 150), it is still present.
...
-   **BP Statistic = 27.78, p-value = 0.02299**:

    -   The null hypothesis of **homoscedasticity** (constant variance) is **rejected** (p-value \< 0.05).

    -   However, the test statistic (BP = 27.78) is significantly lower than before (BP = 64.976), indicating a **reduction in heteroscedasticity** after the transformation.
...
```{r}
lambda = 0.4
model_transf_2nd_order <- lm(((price^(lambda)-1)/(lambda))~distance + surge_multiplier + I(distance^2) + name,data=refined_data)
par(mfrow = c(1, 2))
...
-   **Result**:
    ...
    -   **BP Statistic = 28.591**, **p-value = 0.01186**.
...
```{r}
summary(model_transf_2nd_order)
```
